...
const Header = ({ onSearch }) => {
  ...
  <input 
    ...
    className={`absolute right-0 top-1/2 -translate-y-1/2 rounded-full border border-primary/50 bg-white px-3 py-1 text-xs text-dark outline-none shadow-sm dark:bg-dark dark:text-white transition-all duration-300 z-50 ${
      isSearchVisible ? "w-32 md:w-40 opacity-100 visible" : "w-0 opacity-0 invisible"
    }`}
    autoFocus={isSearchVisible}
  />
  <button 
    onClick={() => setIsSearchVisible(!isSearchVisible)} 
    className={`relative z-[60] flex h-8 w-8 items-center justify-center rounded-full text-dark hover:bg-gray-100 dark:text-white dark:hover:bg-white/10 transition-all ${isSearchVisible ? 'text-primary' : ''}`}
  >
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      <line x1="21" y1="21" x2="16.65" y2="16.65"></line>
            ...
            <div className="hidden items-center lg:flex space-x-3">
              <Link to="/login" className="group relative overflow-hidden rounded-md px-6 py-3 text-base font-medium text-dark dark:text-white transition-all">
              ...
              <Link to="/signup" className="group relative overflow-hidden rounded-md border border-primary px-7 py-3 text-base font-medium text-primary transition-all duration-300 hover:text-white">
